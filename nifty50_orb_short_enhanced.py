...
def calc_charges_short(entry, exit_, qty):
    ...
    def _as_fraction(rate):
        """
        Accept both decimal-form rates (0.00025) and percent-form inputs (0.025).
        If a rate looks too large for brokerage tax fractions, scale it down by 100.
        """
...
def run_short_backtest():
    ...
    if not trades_df.empty:
        ...
        trade_span_days = (
            (pd.to_datetime(trades_df["Date"]).max() - pd.to_datetime(trades_df["Date"]).min()).days + 1
        )
